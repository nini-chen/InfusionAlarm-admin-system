...
<script lang="ts" setup>
  ...
  import { patientRegisterApi, patientEditApi, getPatientDetailApi } from '/@/api/patients/bedExtension';
  ...
  const recordData = ref({} as ICUBedListRecord);
  ...
  async function onDataReceive(data) {
    ...
    if (isUpdate) {
      // 编辑：获取详情，将数据回填到表单
      const res = await getPatientDetailApi({
        partition_id: record.partition_id,
        bed_id: record.bed_id,
      });
      bedId.value = record.bed_id;
      const {
        // bed_id,
        hospital_bed_name,
        hospital_room_name,
  ...
  async function handleSubmit() {
    try {
      ...
      const data = {
        ...values,
        partition_id: recordData.value.partition_id,
        bed_id: bedId.value,
        is_operation,
        ...
        inconfig_5: values.inconfig_5 || '0',
