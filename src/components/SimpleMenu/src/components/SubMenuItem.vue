  ...
  export default defineComponent({
    ...
    setup(props) {
      ...
      const getSubClass = computed(() => {
        const isActive = rootProps.activeName.includes(props.name);
        return [
          `${prefixCls}-submenu-title`,
          {
            [`${prefixCls}-submenu-active`]: getCollapse.value && isActive,
            [`${prefixCls}-submenu-active-border`]: getCollapse.value && isActive && level === 0,
